...
inits' :: [a] -> [[a]]
inits' xs = initsHelper xs [] where
    initsHelper :: [a] -> [a] -> [[a]]
    initsHelper [] xs = [reverse xs]
    initsHelper (x:xs) ys = reverse ys : initsHelper xs (x:ys)
-- A függvény a Data.List modulban található!
...
-- A listáról feltehető, hogy véges.
mergeSort :: Ord a => [a] -> [a]
mergeSort xs = merge (mergeSort fs) (mergeSort sn) where
    (fs, sn) = slpitIntoTwo xs

    slpitIntoTwo :: [a] -> ([a], [a])
    slpitIntoTwo [] = ([],[])
    slpitIntoTwo [x] = ([x], [])
    slpitIntoTwo (x:y:xs) = (x:fs, y:sn) where 
        (fs, sn) = slpitIntoTwo xs
...
insertions :: b -> [b] -> [[b]]
insertions e [] = [[e]]
insertions e (x:xs) = (e:x:xs) : [x:y |y <- insertions e xs]
...
permutations' :: [a] -> [[a]]
permutations' [] = [[]]
permutations' (x:xs) = concat [insertions x y | y <- permutations' xs]
-- A függvény a Data.List modulban található!
...
person2 :: (Name, Adress, Phone, Email, Age)
