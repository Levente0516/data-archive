...
-- Definiáld a filter'' függvényt rekurzivan.ű
...
mapFilter :: [a] -> (a -> b) -> (a -> Bool) -> [b]
mapFilter [] _ _ = []
mapFilter (x:xs) f pre
  | pre x = (f x) : mapFilter xs f pre
  | otherwise = mapFilter xs f pre

mapFilter' :: [a] -> (a -> b) -> (a -> Bool) -> [b]
mapFilter' xs f pre = [f x | x <- xs, pre x]
...
indexedZipWith f xs ys = indexedZipWithHelper 0 f xs ys where
  indexedZipWithHelper :: Integral i => i -> (i -> a -> b -> c) -> [a] -> [b] -> [c]
  indexedZipWithHelper i f [] _ = []
  indexedZipWithHelper i f _ [] = []
  indexedZipWithHelper i f (x:xs) (y:ys) = (f i x y) : indexedZipWithHelper (i + 1) f xs ys  
...
findIndex' :: (Enum a, Num a) => (b -> Bool) -> [b] -> Maybe a
findIndex' f xs
  | isNothing seged = Nothing
  | otherwise = Just (fst (fromJust seged)) where
    seged = find' (\ (id, x) -> f x) (zip [0..] xs)
