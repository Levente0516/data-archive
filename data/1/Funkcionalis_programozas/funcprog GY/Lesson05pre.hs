...
numberWords' text = numberWordsHelper (words text) 1
  where
    numberWordsHelper :: Num a => [String] -> a -> [(a, String)]
    numberWordsHelper [] _ = []
...
intersperse' :: a -> [a] -> [a]
intersperse' _ [] = []
intersperse' sep (x:xs) = x : intersperseHelper sep xs
  where
    intersperseHelper _ [] = []
    intersperseHelper sep (x:xs) = sep : x : (intersperseHelper sep xs)
...
showList' xs = '[' : showListHelper' xs ++ "]" where 
  showListHelper' [] = ""
  showListHelper' [x] = show x
  showListHelper' (x:xs) = show x ++ ',' : showList' xs
...
reverse' :: [a] -> [a]
reverse' xs = reverseHelper xs  [] where
  reverseHelper :: [a] -> [a] -> [a]
  reverseHelper [] xs = xs
...
insertAt n e xs = take' n xs ++ e : drop' n xs 
