...
numbersMadeOfThreesAndOne = iterate (\x -> x*10+21) 1
...
maximumOfMinimums xs = maximum (map minimum xs)
...
firstLetters xs = unwords (map(\ x -> [head x])(words xs))
...
until' pred f s = head (dropWhile (not . pred)(iterate f s))
