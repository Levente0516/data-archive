...
elemIndex' (x:xs) e
  | x == e = Just 0
  | otherwise = add (elemIndex' xs e)(Just 1)
...
select First x y = Left' x
select Second x y = Right' y
...
doubleHeadErrorOr [] = Left "Üres a lista"
doubleHeadErrorOr ([]:_) = Left "Elso lista üres"
...
--data BinTree a = Leaf a | Node (BinTree a) a (BinTree a) deriving (Show, Eq)

-- Adjuk meg a fenti példában látható fát lineáris jelöléssel (tehát Leaf-ek és Node-ok függvényében)!
--t :: Num a => BinTree a
--t = t = Node (Node (Node (Leaf 10) 1 (Leaf 20)) 3 (Leaf 0)) 4 (Node (Leaf 3) 2 (Node (Leaf 5) 7 (Leaf 9)))
